    ...
    public class Hud : MonoBehaviour
    {
        ...
        private void ControllerManager_OnKeyDown(Vector3 eulerAngle) {
            for(int i = tilesContainerContainer.transform.childCount - 1; i >= 0; i--) {
                ...
                tilesContainerContainer.transform.GetChild(i).transform.GetChild(0).eulerAngles = new Vector3(0,0 , eulerAngle.y ); 
            }
        ...
        public void Init() {
            ...
            for(int i = Player.inventory.Count - 1; i >= 0; i--) {
                ...
                tiles = Instantiate(lInventory.Tile, lTilesContainer.GetChild(index).transform.GetChild(0).transform.GetChild(1).transform) ;
                listOfTilesTransform.Add(lTilesContainer.GetChild(index).transform.GetChild(0).transform.GetChild(1).transform);
            
                lTilesContainer.GetChild(index).transform.GetChild(1).GetComponent<Text>().gameObject.SetActive(true);
                lTilesContainer.GetChild(index).transform.GetChild(0).transform.GetChild(0).GetComponent<Image>().gameObject.SetActive(true);
                lTilesContainer.GetChild(index).transform.GetChild(1).GetComponent<Text>().text = lInventory.TilesList.Count.ToString();
                ...
                Tween.LocalScale(lTilesContainer.GetChild(i).transform.GetChild(0).transform.GetChild(1).transform, scaleNormal, 0.6f, 0.2f*i, anim);

            }
            Tween.LocalScale(tilesContainerContainer.transform.GetChild(0).transform.GetChild(0).transform.GetChild(1).transform.transform, scaleOnClick, 0.3f, 1f, Tween.EaseOutBack);
        }

        private void Player_OnIndexChange(int index) {
           Tween.LocalScale(tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(0).transform.GetChild(1).transform, scaleOnClick, 0.2f, 0, Tween.EaseInOutBack);
        }

        private void Player_OnUpdateInventory(int index) {
            tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - player.index).transform.GetChild(1).GetComponent<Text>().text = index.ToString();
        }

        private void Player_OnRecupTile(int index) {
            ResetScaleHud(); 
            indexOnRecup = index;
            Tween.LocalScale(tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(0).transform.GetChild(0), scaleOnClick, 0.3f, 0.3f, Tween.EaseInBack, Tween.LoopType.None, () => { tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(0).transform.GetChild(1).gameObject.SetActive(true); });
            tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(1).GetComponent<Text>().gameObject.SetActive(true);
        ...
        private void Player_OnInventoryEmpty(int index) {
            Tween.LocalScale(tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(0).transform.GetChild(0), Vector3.zero, 0.2f, 0, Tween.EaseInBack,Tween.LoopType.None, null,()=> { tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(0).transform.GetChild(1).gameObject.SetActive(false); });
            tilesContainerContainer.transform.GetChild(Player.inventory.Count - 1 - index).transform.GetChild(1).GetComponent<Text>().gameObject.SetActive(false);
            indexOnEmpty = index;
        ...
        protected void ChangeIndexOfInventory(int index) {
            if(!tilesContainerContainer.transform.GetChild(index).transform.GetChild(0).transform.GetChild(1).gameObject.activeSelf) return;
            ResetScaleHud(); 
           
            player.index = (Player.inventory.Count - 1) - index;
            Tween.LocalScale(tilesContainerContainer.transform.GetChild(index).transform.GetChild(0).transform.GetChild(1).transform, scaleOnClick, 0.2f, 0, Tween.EaseInOutBack); 
        ...
        public void ResetHud() {
            ...
            for(int i = tilesContainerContainer.transform.childCount - 1; i >= 0; i--) {
                tilesContainerContainer.transform.GetChild(i).gameObject.SetActive(false); 
                tilesContainerContainer.transform.GetChild(i).transform.GetChild(1).GetComponent<Text>().gameObject.SetActive(false);
                ...
                tilesContainerContainer.transform.GetChild(i).transform.GetChild(0).rotation =  Quaternion.identity;
        ...
        public void SetActiveSlots() {
            for(int i = 0; i < Player.inventory.Count; i++) {
                tilesContainerContainer.transform.GetChild(i).gameObject.SetActive(true); 
            }
        }
